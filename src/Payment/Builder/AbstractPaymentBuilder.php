...
abstract class AbstractPaymentBuilder implements PaymentBuilderInterface
{
    ...
    protected function getTpayTransactionConfig(SyncPaymentTransactionStruct $transaction, OrderEntity $order, CustomerEntity $customer, SalesChannelContext $salesChannelContext): TpayTransactionConfigStruct
    {
        ...
        $tpayTransactionConfig
            ...
            ->setBuyer($customer)
            ->setResultUrl($this->assembleResultUrl($token))
            ->setReturnUrl($this->assembleReturnUrl($token))
    ...
    private function assembleResultUrl(string $token): string
    {
        $parameter = ['_sw_token' => $token];
    ...
    private function assembleReturnUrl(string $token): string
    {
        ...
        return $this->router->generate('tpay.finalize.transaction', $parameter, UrlGeneratorInterface::ABSOLUTE_URL);
    }
}
